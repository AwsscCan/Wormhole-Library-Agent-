...
| `lib/composition.ts` | P01 组合绑定入口 `ensureAppComposition()`（绑 P01 principal、P02 目录端口、P04 记忆端口与写作端口） |
...
- `instrumentation.ts` 在 Node runtime 启动时调用 `ensureAppComposition()`；P04 `defaultMemoryReadPort` 被绑定到 P05 workbench 消费面。
...
### Final-integration 整合记录（2026-08-28）

`cc69c89` 没有作为整分支直接合入，因为该分支历史会删除已验收的 P05 Workbench。当前 final-integration 只移植 P04 通过项：

- `lib/research/memory/*`、P04 memory 单测、`scripts/experiment-memory-rag.ts` 与 `202608280001_package04_memory` 迁移。
