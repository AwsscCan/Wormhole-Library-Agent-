...
{
  const aliceNote = recordLearningEvent({ ownerId: "member:alice", sessionId: "sA", kind: "note", conceptId: "rag", text: "Alice private notes on retrieval augmented generation" });
  recordLearningEvent({ ownerId: "member:bob", sessionId: "sB", kind: "note", conceptId: "rag", text: "Bob private notes on retrieval augmented generation" });

  const aliceHits = searchPrivateMemory({ ownerId: "member:alice", query: "retrieval augmented generation", limit: 10 });
  ...
  const aliceSnippetId = searchPrivateMemory({ ownerId: "member:alice", sessionId: "sA", query: "retrieval augmented", limit: 5 })[0]?.id;
  deleteMemorySnippet("member:alice", aliceSnippetId ?? aliceNote.id);
  const afterDelete = searchPrivateMemory({ ownerId: "member:alice", query: "retrieval augmented generation", limit: 10 });
  console.log(row(["member:alice（删除后）", afterDelete.length, "-", "删除后不再召回 ✓"]));

  // session scoping
  const sessionHits = searchPrivateMemory({ ownerId: "member:alice", sessionId: "sA", query: "anything", limit: 10 });
...
console.log(row(["模式", "召回数", "Top1分数", "Top1来源/时间保留", "结论"]));
{
  ...
  const lexical = searchPrivateMemory({ ownerId: "member:carol", query, limit: 5, mode: "lexical-only" });
  const hybrid = searchPrivateMemory({ ownerId: "member:carol", query, limit: 5 });
  ...
  console.log(row(["会话过滤", searchPrivateMemory({ ownerId: "member:carol", sessionId: "s1", query, limit: 5 }).length, "-", "-", "session 作用域生效 ✓"]));
}
...
{
  const snippet = addMemorySnippet({
    ownerId: "member:carol",
    sessionId: "s1",
    kind: "excerpt",
    sourceId: "ev-exc-1",
  ...
  const supported = answerFromSelectedMaterial({ ownerId: "member:carol", question: "what do multi-agent systems require", selectedSourceIds: [snippet.id] });
  console.log(row(["有选定资料", supported.status, supported.citations.length, "返回摘录引用 ✓"]));

  const unsupported = answerFromSelectedMaterial({ ownerId: "member:carol", question: "quantum error correction thresholds", selectedSourceIds: [snippet.id] });
  console.log(row(["无支持资料", unsupported.status, unsupported.citations.length, "明确未知，不编造 ✓"]));

  const empty = answerFromSelectedMaterial({ ownerId: "member:carol", question: "multi-agent coordination", selectedSourceIds: [] });
...
section("必交失败样例");
{
  ...
  const aliceSnippet = addMemorySnippet({ ownerId: "member:alice", sessionId: "sX", kind: "note", sourceId: "ev-x", text: "alice confidential wormhole seed material" });
  const bobAnswer = answerFromSelectedMaterial({ ownerId: "member:bob", question: "confidential wormhole seed", selectedSourceIds: [aliceSnippet.id] });
  console.log(`- 跨owner拒绝：bob 用 alice 的 snippet id 提问 → status=${bobAnswer.status}（拒绝）✓`);
  const bobHits = searchPrivateMemory({ ownerId: "member:bob", query: "confidential wormhole seed material", limit: 5 });
