...
const storeFile = path.join(output, "experiment-store.json");
...
async function main() {
let sequence = 0;
const service = new ResearchSessionService(new FileResearchSessionStore(storeFile), {
...
for (const [index, topic] of topics.entries()) {
  ...
  const restarted = new ResearchSessionService(new FileResearchSessionStore(storeFile));
...
const experiment = {
  packageVersion: "v3.2-p03-schema-1",
...
fs.writeFileSync(path.join(output, "failure-samples.json"), JSON.stringify([
  ...
  { code: "CORRUPT_RECOVERY", recovery: "rename to .corrupt and start empty" },
], null, 2));
