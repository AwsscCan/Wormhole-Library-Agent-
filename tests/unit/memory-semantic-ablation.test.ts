...
function mockMalformedOllamaFetch(): typeof fetch {
  return (async () => {
    return new Response(JSON.stringify({ embeddings: [[]] }), { status: 200 });
  }) as typeof fetch;
}

function mockDimensionMismatchFetch(): typeof fetch {
  let calls = 0;
  return (async () => {
    calls += 1;
    const vector = calls === 1 ? [1, 0, 0, 0] : [1, 0, 0];
...
describe("default semantic path (real embedding provider)", () => {
  it("recalls a true English synonym with no token or character overlap (car servicing → automobile maintenance)", async () => {
    ...
    expect(getSemanticEmbedderStatus()).toMatchObject({ status: "semantic", degraded: false });
  });
...
describe("explicit degradation when the embedding model is unavailable", () => {
  it("falls back to char n-gram and reports degraded=true, without faking semantics", async () => {
    ...
    expect(getSemanticEmbedderStatus()).toMatchObject({ status: "degraded", provider: "char-ngram" });
  });

  it("falls back explicitly when Ollama returns a malformed vector", async () => {
    setSemanticEmbedderForTests(createSemanticEmbedder({ useOllama: true, fetchImpl: mockMalformedOllamaFetch() }));
    await addMemorySnippet({
      ownerId: "member:alice", sessionId: "s1", sourceId: "ev-auto", kind: "note",
      text: "automobile maintenance",
    });

    expect(getSemanticEmbedderStatus()).toMatchObject({
      status: "degraded",
      degraded: true,
      provider: "char-ngram",
    });
  ...
  it("reports unavailable when an established semantic provider changes vector dimensions", async () => {
    setSemanticEmbedderForTests(createSemanticEmbedder({
      useOllama: true,
      fetchImpl: mockDimensionMismatchFetch(),
      fallback: null,
    }));
    await addMemorySnippet({
      ownerId: "member:alice", sessionId: "s1", sourceId: "ev-dim", kind: "note",
      text: "automobile maintenance",
    });

    await expect(
      searchPrivateMemory({ ownerId: "member:alice", query: "car servicing", limit: 5 }),
    ).rejects.toThrow(/dimension mismatch|fallback/i);
    expect(getSemanticEmbedderStatus()).toMatchObject({
      status: "unavailable",
      degraded: true,
