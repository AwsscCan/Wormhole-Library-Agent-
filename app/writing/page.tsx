...
import { Check, ChevronRight, Download, FileText, Flag, ListChecks, Pause, Play, RotateCcw, Search, Settings2, Sparkles, Upload } from "lucide-react";
...
export default function WritingPage() {
  ...
  return <div className="mx-auto max-w-[1440px] space-y-4">
    <header className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="flex items-center gap-2 font-display text-xl text-ivory"><FileText className="h-5 w-5 text-copper" />写作工作台</h1><p className="mt-1 text-sm text-steel">按材料、参数、检查点、模型和产物顺序运行可恢复的证据写作流程。</p></div><div className="flex items-center gap-3"><Link href="/research" className="text-xs text-pulse hover:underline">返回研究工作区</Link><Link href="/review" className="text-xs text-copper hover:underline">文献综述入口</Link></div></header>
