...
export default function LegacyMapMigrationPage({ params }: { params: Promise<{ interactionId: string }> }) {
  ...
  useEffect(() => {
    fetch("/api/research/migrations", {
      ...
      if (!response.ok) throw new Error(data.error?.code === "EXPIRED_INTERACTION" ? "这次旧检索已过期，但你可以创建新会话后从主题节点重新搜索。" : data.error?.message ?? "无法迁移旧地图");
      router.replace(`/research/${data.sessionId}/map`);
  ...
  return <div className="flex h-64 items-center justify-center gap-2 font-mono text-xs uppercase tracking-[0.18em] text-steel-dim"><Loader2 className="h-4 w-4 animate-spin text-pulse" />upgrading legacy map…</div>;
}
