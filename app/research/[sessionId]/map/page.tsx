...
export default async function ResearchMapPage({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  try {
    ...
    return <div className="space-y-3">
      ...
      <PersonalGraphWorkspace initialSession={session} initialGraph={mergePersonalGraph(systemGraph, session.personalGraph)} publicGraphHash={hashPublicGraph(systemGraph)} />
    </div>;
  } catch {
