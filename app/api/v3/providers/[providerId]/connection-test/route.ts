...
export async function POST(request: Request, context: { params: Promise<{ providerId: string }> }) {
  ...
  try { if (!await consumeConnectionTest(principal.id, providerId)) return privateWritingResponse(new Response(JSON.stringify({ error: { code: "BAD_REQUEST", message: "Connection test is temporarily rate limited" } }), { status: 429, headers: { "content-type": "application/json", "Retry-After": "60" } }), principal); const { provider, apiKey } = await getOwnedProviderSecret(principal, providerId); await testProviderConnection(provider, apiKey); return privateWritingResponse(new Response(JSON.stringify({ ok: true }), { headers: { "content-type": "application/json" } }), principal); }
