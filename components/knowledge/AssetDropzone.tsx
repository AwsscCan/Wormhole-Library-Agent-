...
type Asset = { id: string; originalName: string; mimeType: string; byteSize: number; retention: "temporary" | "library"; expiresAt?: string; extractionStatus: string };
...
export function AssetDropzone() {
  ...
  return <section className="border border-ink-border bg-ink-panel p-4">
    ...
    {assets.length > 0 && <div className="mt-3 divide-y divide-ink-border border-y border-ink-border">{assets.map((asset) => <div key={asset.id} className="flex items-center gap-2 py-2"><div className="min-w-0 flex-1"><p className="truncate text-xs text-ivory">{asset.originalName}</p><p className="text-[10px] text-steel-dim">{size(asset.byteSize)} · {asset.retention === "library" ? "私有知识库" : `至 ${asset.expiresAt ? new Date(asset.expiresAt).toLocaleDateString() : "30 天后"}`} · {asset.extractionStatus}</p></div><button type="button" aria-label={`删除 ${asset.originalName}`} onClick={() => void remove(asset.id)} className="p-1 text-steel hover:text-rosewood"><Trash2 className="h-3.5 w-3.5" /></button></div>)}</div>}
  </section>;
}
