  ...
  return <div className="space-y-4">
    ...
    {activeView === "reading" && <div className="grid gap-4 xl:grid-cols-[360px_1fr]">
      ...
      <section className="space-y-3">{recommendations.map((item) => <article key={item.id} className="rounded-lg border border-ink-border bg-ink-panel p-4">
        ...
        {(item.memoryBoost ?? 0) > 0 && <div className="mt-2 border-l-2 border-copper/60 bg-copper-faint/15 px-3 py-2 text-[10px]"><div className="flex items-center justify-between text-copper"><span>混合记忆参与排序</span><span className="font-mono">+{(item.memoryBoost ?? 0).toFixed(2)}</span></div><p className="mt-1 leading-relaxed text-steel">{item.decisionTrace?.memorySnippetIds.length ?? 0} 条私有 RAG 片段 · {item.decisionTrace?.preferenceIds.length ?? 0} 条长期偏好{item.explanationContext ? ` · 来源 ${item.explanationContext}` : ""}</p></div>}
