...
export function ProviderSettings() {
  ...
  return <div className="mx-auto max-w-5xl space-y-4">
    ...
    <Panel><PanelHeader icon={Languages} title="workspace · 外观与语言" accent="copper" /><PanelBody className="space-y-5"><ThemeSettings onSaved={setStatus} /><div className="border-t border-ink-border pt-4"><label className="block max-w-md text-xs text-steel"><span className="mb-2 block">检索语言偏好</span><select value={language} onChange={(event) => saveLanguage(event.target.value)} className="h-10 w-full border border-ink-border bg-ink-raise px-3 text-sm text-ivory">{languageOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select><span className="mt-2 block text-[10px] leading-relaxed text-steel-dim">语言偏好会同时影响联邦检索排序和写作证据发现。</span></label></div></PanelBody></Panel>
